...
contract FundMeTest is ZkSyncChainChecker, CodeConstants, StdCheats, Test {
    ...
    function setUp() external {
        ...
        vm.deal(USER, STARTING_USER_BALANCE); //not to put in original smart contract
    }
    ...
    function testFundFailsWithoutEnoughETH() public skipZkSync{
        vm.expectRevert(); // hey, the next line should revert otherwise this fails
        fundMe.fund();
    }

    function testFundUpdatesFundedDataStructure() public skipZkSync{
        vm.startPrank(USER); //the next txn will be sent by USER, working in foundry only
